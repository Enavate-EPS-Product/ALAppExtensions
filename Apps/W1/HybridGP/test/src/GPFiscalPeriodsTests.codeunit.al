codeunit 139701 "GP Fiscal Periods Tests"
{
    ...
    begin
        ...
        CreateGPFiscalPeriodData(MSFTSY40101Table);
    ...
    begin
        ...
        // [GIVEN] Some records are created in the staging table
        CreateNonCalendarPeriods(MSFTSY40101Table);

        // [WHEN] Fiscal Period migration code is called
        Migrate(MSFTSY40101Table);

        // [THEN] Accounting Periods are created
        //     6 years with 12 periods per year = 72 + 1 (start of next Accounting period) = 73
        AccountingPeriod.Reset();
        Assert.RecordCount(AccountingPeriod, 73);
        ...
        AccountingPeriod.Reset();
        AccountingPeriod.SetFilter("New Fiscal Year", '1');
        Assert.RecordCount(AccountingPeriod, 7);
    end;
    ...
    begin
        ...
        // [GIVEN] Some records are created in the staging table
        CreateNonTypicalPeriods(MSFTSY40101Table);

        // [WHEN] Fiscal Period migration code is called
        Migrate(MSFTSY40101Table);

        // [THEN] Accounting Periods are created
        //     3 years with 6 periods + 2 years with 24 periods + 1 year with 12 periods = 78 + 1 (start of next Accounting period) = 79
        AccountingPeriod.Reset();
        Assert.RecordCount(AccountingPeriod, 79);
    ...
    [Test]
    [TransactionModel(TransactionModel::AutoRollback)]
    procedure TestGPFiscalPeriodsNonTypicalYears2()
    var
        AccountingPeriod: Record "Accounting Period";
        MSFTSY40101Table: Record MSFTSY40101;
        MyDate: Date;
    begin
        ...
        AccountingPeriod.Reset();
        MyDate := 20200601D;
        AccountingPeriod.SetFilter("Starting Date", Format(MyDate));
        Assert.RecordCount(AccountingPeriod, 1);
    end;
    ...
    local procedure CreateGPFiscalPeriodData(var MSFTSY40101Table: Record MSFTSY40101)
    begin
    ...
    local procedure CreateNonCalendarPeriods(var MSFTSY40101Table: Record MSFTSY40101)
    begin
        MSFTSY40101Table.Init();
        MSFTSY40101Table.YEAR1 := 2020;
        MSFTSY40101Table.NUMOFPER := 12;
    ...
    local procedure CreateNonTypicalPeriods(var MSFTSY40101Table: Record MSFTSY40101)
    begin
        MSFTSY40101Table.Init();
        MSFTSY40101Table.YEAR1 := 2020;
        ...
        MSFTSY40100Table.YEAR1 := 2020;
        MSFTSY40100Table.SERIES := 2;
        MSFTSY40100Table.PERIODID := 1;
        MSFTSY40100Table.PERIODDT := CreateDateTime(20200101D, 0T);
        MSFTSY40100Table.PERDENDT := CreateDateTime(20200131D, 0T);
        ...
        MSFTSY40100Table.PERIODDT := CreateDateTime(20200201D, 0T);
        MSFTSY40100Table.PERDENDT := CreateDateTime(20200229D, 0T);
        ...
        MSFTSY40100Table.PERIODDT := CreateDateTime(20200301D, 0T);
        MSFTSY40100Table.PERDENDT := CreateDateTime(20200331D, 0T);
        ...
        MSFTSY40100Table.PERIODDT := CreateDateTime(20200401D, 0T);
        MSFTSY40100Table.PERDENDT := CreateDateTime(20200430D, 0T);
        ...
        MSFTSY40100Table.PERIODDT := CreateDateTime(20200501D, 0T);
        MSFTSY40100Table.PERDENDT := CreateDateTime(20200531D, 0T);
